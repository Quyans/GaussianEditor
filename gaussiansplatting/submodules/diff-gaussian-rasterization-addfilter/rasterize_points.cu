...
RasterizeGaussiansCUDA(
    ...
    const bool filter_switch,
    const torch::Tensor &filter_rectangle,
    const bool debug) {
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        filter_switch,
        filter_rectangle.contiguous().data<float>(),
		    masks.contiguous().data<int>(),
        out_color.contiguous().data<float>(),
        out_depth.contiguous().data<float>(), radii.contiguous().data<int>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, radii, masks, geomBuffer,
