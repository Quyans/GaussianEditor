...
class WebUI:
    def __init__(self, cfg) -> None:
        ...
        with self.server.add_gui_folder("Semantic Tracing"):
            ...
            self.add_negative_points = self.server.add_gui_checkbox(
                "Add Negative Points", initial_value=False
            )
    ...
    def click_cb(self, pointer):
        if self.sam_enabled.value and self.add_sam_points.value:
            ...
            if self.add_negative_points.value:
                self.add_points3d_neg(self.camera, click_pos)
            else:
    ...
    def update_sam_mask_with_point_prompt(
        ...
        for idx in tqdm(view_index):
            ...
            if len(points3d_neg) > 0:
                points2ds_neg = project(cur_cam, points3d_neg)
            ...
            if len(points3d_neg) > 0:
                neg_labels = np.array([0] * points2ds_neg.shape[0], dtype=np.int64)
                point_labels = np.concatenate((point_labels, neg_labels), axis=0)
                
                points2ds = torch.cat((points2ds, points2ds_neg), dim=0)
            ...
            self.gaussian.apply_weights(
                cur_cam, weights, weights_cnt, mask.to(torch.float32)   #weights_cnt 每做一次都会动
            )
            masks.append(mask)
    ...
    def sam_predict(self, image, cam):
        ...
        _all_points2ds = _points2ds
        
        _points2ds_neg = []

        point_labels = np.array([1] * _points2ds.shape[0], dtype=np.int64)
        if len(self.points3d_neg) > 0:
            _points2ds_neg = project(cam, self.points3d_neg)

            neg_labels = np.array([0] * _points2ds_neg.shape[0], dtype=np.int64)
            point_labels = np.concatenate((point_labels, neg_labels), axis=0)
            
            _all_points2ds = torch.cat((_points2ds, _points2ds_neg), dim=0)
        
        _mask, _, _ = self.sam_predictor.predict(
            point_coords=_all_points2ds.cpu().numpy(),
            point_labels=point_labels,
            box=None,
            multimask_output=False,
        )
        ...
        return _mask.squeeze(), _points2ds, _points2ds_neg
